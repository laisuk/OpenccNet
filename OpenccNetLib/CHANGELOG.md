...
- Removed the retired `DictSlot.JPVariants` and `DictSlot.JPVariantsRev` enum members. Numeric values 16 and 17 remain
  intentionally unused; all surviving `DictSlot` numeric values are unchanged.
- Removed the `PeterO.Cbor` dependency and its transitive object-mapping/reflection requirements from dictionary CBOR
  serialization.
...
- Make `DictionaryLib.FromZstd(string)` public for loading an independent dictionary from an explicitly supplied
  external Zstd file.
...
- Migrate dictionary CBOR serialization from `PeterO.Cbor` object mapping to an explicit `System.Formats.Cbor`
  reader/writer implementation. The new codec avoids reflection-based CLR object mapping and is trimming- and
  NativeAOT-friendly.
...
- Removed CBOR trimming and NativeAOT analysis warnings caused by the reflection-capable `PeterO.Cbor` object-mapping
  implementation. CBOR dictionary loading and generation now use explicit `System.Formats.Cbor` encoding and decoding.
...
- `DictionaryLib.FromZstd(string)` loads an explicitly supplied external Zstd file and returns an independent dictionary
  without changing the active global dictionary provider or conversion-plan cache.
- Existing CBOR dictionary files generated by the previous `PeterO.Cbor` implementation remain readable by the new
  `System.Formats.Cbor` loader.
- Newly generated CBOR dictionaries retain the established legacy wire schema and remain readable by older
  PeterO.Cbor-based OpenccNetLib versions, preserving dictionary portability when users switch dictionary files between
  newer and older installations.
...
- Remove the redundant `DictionaryLib.LoadJsonCompressed(string)` API; use `DictionaryLib.FromZstd(string)` to load
  external Zstandard-compressed dictionaries.
...
- Added canonical custom-dictionary token parsing using `<slot>:<append|override>:<path>`. The public
  ...
  enumeration. Numeric and unknown slots are rejected, while Windows and other colon-containing paths remain supported.
